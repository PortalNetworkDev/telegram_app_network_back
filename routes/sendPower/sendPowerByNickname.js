...
export default async function (fastify) {
    ...
    { schema, onRequest: [fastify.auth] }, async (request, reply) => {
        ...
        if (recipientName && senderId) {
            ...
            try {
                // @ts-ignore: Unreachable code error
                await fastify.models_mining_power.addPowerBalance(recipientInstance[0].id, sendPowerAmount);
            }
            catch (error) {
            ...
            try {
                // @ts-ignore: Unreachable code error
                await fastify.models_mining_power.subtractPowerBalance(senderInstance.id, sendPowerAmount);
            }
            catch (error) {
