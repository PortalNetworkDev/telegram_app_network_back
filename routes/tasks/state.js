...
export default async function (fastify, opts) {
  fastify.get(
    ...
    async function (request, reply) {
      const user = await fastify.models_user.getUser(
      ...
      const en_cats = await fastify.utils.csvParser(
        "./db/catstranslate_en.csv"
      );
      const en_tasks = await fastify.utils.csvParser(
        ...
        for (let index = 0; index < tasks.length; index++) {
          ...
          if (task.category_id == cat.id) {
            ...
            if (task.type == "referal") {
              task.description =
                task.description +
                `<br><br>${subscribeUsersText}: <b>${await fastify.models_user.countReferalUsers(
                  user.id
                )}</b>`;
            ...
            if (task.type == "checkLiquidity") {
              const days =
                await fastify.models_balance_history.getHoleInHistoryPoolBalance(
                  user.id
                );
              task.description =
                task.description +
                `<br><br>${daysInPoolText}: <b>${days.length}</b>`;
            }
