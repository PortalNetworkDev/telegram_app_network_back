...
module.exports = async function (fastify, opts) {
  fastify.get('/data',{onRequest: [fastify.auth]}, async function (request, reply) {
    ...
    const obj = {
      ...
      price_rize_generator: 2**(Math.round((data.generator_level)/2))*Number(fastify.config.stepGeneratorPrice),
      power_rize_generator: Number(fastify.config.stepGeneratorLim),
      price_rize_battery: 2**(Math.round((data.battery_level+1)/2))*Number(fastify.config.stepBatteryPrice),
      power_rize_battery: Number(fastify.config.stepBatteryCap)*Math.round(1.2**data.battery_level),
      recovery_power_lim: Number(fastify.config.recoveryGeneratorLim),
      multitab: data.multitab,
      price_rize_multitab: 2**(Math.round((data.multitab-1)/2))*Number(fastify.config.stepMultitabPrice),
      level: data.level
