...
export default createPlugin<FastifyPluginAsync>(async function (fastify, opts) {
  ...
  async function checkBuyTokenStonFi(wallet: string) {
    let have = false;
    let tryCount = 0;
  ...
  async function getFarmingNft(wallet: string) {
    let poolbalance = 0;
    let tryCount = 0;
  ...
  async function getBalances(wallet: string) {
    let jettonbalance = 0;
    let poolbalance = 0;
    let tryCount = 0;
      ...
      if (_req?.status == 200) {
        ...
        for (let index = 0; index < req.balances.length; index++) {
          ...
          if (balance.jetton.address == fastify.config.lptokenaddress) {
            poolbalance = Number(tonweb.utils.fromNano(balance.balance));
          }

          if (balance.jetton.address == fastify.config.jettonaddressraw) {
            jettonbalance = Number(tonweb.utils.fromNano(balance.balance));
          }
        }
      } else {
        jettonbalance = -1;
  ...
  function csvParser<T>(file: string, delimeter = ","):Promise<T[]> {
