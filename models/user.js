...
module.exports = fp(async function (fastify, opts) {
    ...
    const createUser = async (user, tg_token) => {
        ...
        VALUES (?, ?,?,?,?,?,?, ?,?,"",?)`
        let values = [user.id, user.first_name,(user.last_name) ? user.last_name : "", (user.username) ? user.username: "",user.language_code,(user.is_premium) ? user.is_premium: 0,(user.allows_write_to_pm) ? user.allows_write_to_pm : 0, Date.now(), Number(fastify.config.referalreward), tg_token]
